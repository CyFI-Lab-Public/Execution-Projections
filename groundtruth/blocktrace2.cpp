...
VOID Trace(TRACE trace, VOID* v)
{
    ...
    */
    ...
    if (is_valid) {
        ...
        if (is_main) {
            DBGFile << "main, " << IMG_LowAddress(current_image) << endl;
        } else {
            DBGFile << "!!! NOT main !!!" << endl;
        }
    } else {
        DBGFile << "!!! NOT valid !!!" << endl;
    }
...
KNOB< string > KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool", "o", "blocktrace2.out", "specify output file name");
KNOB< string > KnobDebugFile(KNOB_MODE_WRITEONCE, "pintool", "d", "blocktrace2_debug.out", "specify debug file name");
